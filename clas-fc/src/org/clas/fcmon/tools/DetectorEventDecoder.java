...
public class DetectorEventDecoder {
    ...
    public void setTETScaleFactor(double scale) {
    	  this.TETScale = scale;
    }
        ...
        for(DetectorDataDgtz data : detectorData){            
            ...
            for(String table : keysFitter){
                ...
                if(daq.hasEntry(crate,slot,channel)==true){                    
                    ...
                    double tet = (int) daq.getIntValue("tet", crate,slot,channel)*TETScale;
                    ...
                    if(data.getADCSize()>0){
                        for(int i = 0; i < data.getADCSize(); i++){
                            ...
                            if(adc.getPulseSize()>0){
                                ...
                                //        crate + " / " + slot + " / " + channel);
                                try {
                                    extendedFitter.fit(nsa, nsb, (int) tet, 0, adc.getPulseArray());
                                } catch (Exception e) {
