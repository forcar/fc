...
public class JCSG {
    ...
    int[][]       vertices = {{4,0,5,1,7,3,6,2},{6,2,7,3,5,1,4,0},{6,2,7,3,5,1,4,0}};  //CJ vertices -> JCSG vertices
    int                cal = 0;
    ...
    private void processVert() {
        for (int cal=0; cal<3; cal++) {doJCSGVert();doCJVert(); }   	
    }
    ...
    private void doJCSGVert() {   
        ...
        case 2: printJCSG(ecal.getPaddle(isec[1],ilay[1]+15,istr[1])) ; 
        }
    }
    ...
    private void printCJ(ScintillatorPaddle paddle) {
    	for(int j=0; j<8 ; j++) {
    		point1.copy(paddle.getVolumePoint(vertices[ilay[cal]-1][j]));
    		double r0 = Math.sqrt(point1.x()*point1.x()+point1.z()*point1.z());
