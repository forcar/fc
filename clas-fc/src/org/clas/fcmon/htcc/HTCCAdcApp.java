...
public class HTCCAdcApp extends FCApplication {

    EmbeddedCanvas c = this.getCanvas(this.getName()); 
    int ics;
    
    public HTCCAdcApp(String name, HTCCPixels[] htccPix) {
        super(name,htccPix);    
     }
    
    public void updateCanvas(DetectorDescriptor dd) {
        
        double amax = htccPix[0].amax[0];
        int[] ics = {10,10};
...
//        canvas.setStatBoxFontSize(12);
        ...
        H2F h2a = htccPix[ilm].strips.hmap2.get("H2_a_Hist").get(is,1,0); h2a.setTitleY("Sector "+is+" LEFT PMT") ; h2a.setTitleX("LEFT PMT ADC");
        H2F h2b = htccPix[ilm].strips.hmap2.get("H2_a_Hist").get(is,2,0); h2b.setTitleY("Sector "+is+" RIGHT PMT") ; h2b.setTitleX("RIGHT PMT ADC");
        H2F h2c = htccPix[ilm].strips.hmap2.get("H2_a_Hist").get(is,0,0); h2c.setTitleY("Sector "+is+" RING") ; h2c.setTitleX("L+R");
        ...
        h1a = htccPix[ilm].strips.hmap2.get("H2_a_Hist").get(is,1,0).projectionY(); h1a.setTitleX("Sector "+is+" LEFT PMT");  h1a.setFillColor(col0);  
        h1b = htccPix[ilm].strips.hmap2.get("H2_a_Hist").get(is,2,0).projectionY(); h1b.setTitleX("Sector "+is+" RIGHT PMT" ); h1b.setFillColor(col0);  
        h1c = htccPix[ilm].strips.hmap2.get("H2_a_Hist").get(is,0,0).projectionY(); h1c.setTitleX("Sector "+is+" RING" ); h1c.setFillColor(col1);  
        ...
        h1a = htccPix[ilm].strips.hmap2.get("H2_a_Hist").get(is,1,0).sliceY(ic); h1a.setTitleX("LEFT PMT "+(ic+1)+" ADC");  h1a.setFillColor(col0);  
        h1b = htccPix[ilm].strips.hmap2.get("H2_a_Hist").get(is,2,0).sliceY(ic); h1b.setTitleX("RIGHT PMT "+(ic+1)+" ADC" ); h1b.setFillColor(col0);  
        h1c = htccPix[ilm].strips.hmap2.get("H2_a_Hist").get(is,0,0).sliceY(ic); h1c.setTitleX("L+R RING "+(ic+1));   h1c.setFillColor(col2);  
